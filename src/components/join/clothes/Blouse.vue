<template>
  <div class="clothes subContent mauto">
    <div class="content-title mt70">
      <span v-show="!this.Authentication.authenticated">블라우스</span>
      <styleMenu v-show="this.Authentication.authenticated" menuTitle="블라우스"></styleMenu>
    </div>
    ...
    <div class="clothes-form w100 mauto mt30">
      <template v-for="(data, k) in printData">
        <div
          ...
          @click="pickClothes({ type: 'blouse', id: data.code })"
        >
    ...
    <styleButton customCss="float: right;" currentNumber="4"></styleButton>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...mapGetters({
      blouseList: 'signup/getBlouse',
      selected: 'signup/getSelectBlouse',
      Authentication: 'login/Authentication',
      mypageStyleData: 'mypage/getMypageStyleData',
    }),
    printData() {
      const rtn = this.blouseList;
    ...
    if (this.Authentication.authenticated) {
      ...
      if (this.selected.length <= 0) {
        for (let i = 0; i < exceptClothes.length; i += 1) {
          for (let j = 0; j < this.blouseList.length; j += 1) {
            if (exceptClothes[i] === this.blouseList[j].code.toString()) {
              this.pickClothes({ type: 'blouse', id: this.blouseList[j].code });
              break;
            }
          }
        }

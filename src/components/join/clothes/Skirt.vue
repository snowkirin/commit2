<template>
  <div class="clothes subContent mauto">
    <div class="content-title mt70">
      <span v-show="!this.Authentication.authenticated">스커트</span>
      <styleMenu v-show="this.Authentication.authenticated" menuTitle="스커트"></styleMenu>
    </div>
    ...
    <div class="clothes-form w100 mauto mt30">
      <template v-for="(data, k) in printData">
        <div
          ...
          @click="pickClothes({ type: 'skirt', id: data.code })"
        >
    ...
    <styleButton customCss="float: right;" currentNumber="6"></styleButton>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...mapGetters({
      skirtList: 'signup/getSkirt',
      selected: 'signup/getSelectSkirt',
      Authentication: 'login/Authentication',
      mypageStyleData: 'mypage/getMypageStyleData',
    }),
    printData() {
      const rtn = this.skirtList;
  ...
  methods: {
    ...
    moveNext() {
      this.$localStorage.set('S6', JSON.stringify(this.selected));
  ...
  async created() {
    ...
    let localStorage = this.$localStorage.get('S6');
    ...
    if (this.Authentication.authenticated) {
      ...
      if (this.selected.length <= 0) {
        for (let i = 0; i < exceptClothes.length; i += 1) {
          for (let j = 0; j < this.skirtList.length; j += 1) {
            if (exceptClothes[i] === this.skirtList[j].code.toString()) {
              this.pickClothes({ type: 'skirt', id: this.skirtList[j].code });
              break;
            }
          }
        }
      ...
      if (this.selected.length <= 0) {
        for (let i = 0; i < localStorage.length; i += 1) {
          this.pickClothes({ type: 'skirt', id: localStorage[i] });
        }
      }
    }
  },

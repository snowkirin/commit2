<template>
  <div class="clothes subContent mauto">
    ...
    <div class="clothes-form w100 mauto mt30">
      <template v-for="(data, k) in printData">
        <div
          ...
          @click="pickClothes({ type: 'pants', id: data.code })"
        >
    ...
    <styleButton customCss="float: right;" currentNumber="8"></styleButton>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...mapGetters({
      dataList: 'signup/getPants',
      selected: 'signup/getSelectPants',
        ...
        for (let i = 0; i < exceptClothes.length; i += 1) {
          for (let j = 0; j < this.dataList.length; j += 1) {
            if (exceptClothes[i] === this.dataList[j].code.toString()) {
              this.pickClothes({ type: 'pants', id: this.dataList[j].code });
              break;
            }
          }
        }
